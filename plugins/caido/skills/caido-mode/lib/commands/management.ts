...
export async function cmdUpdateScope(scopeId: string, name?: string, allow?: string[], deny?: string[]) {
  const client = await getClient();
...
export async function cmdUpdateFilter(filterId: string, name?: string, query?: string, alias?: string) {
  const client = await getClient();
...
export async function cmdCreateEnv(name: string) {
  const client = await getClient();
  const environment = await client.environment.create({ name, variables: [] });
  console.log(JSON.stringify({ id: environment.id, name: environment.name }, null, 2));
}
...
export async function cmdEnvSet(envId: string, varName: string, value: string) {
  const client = await getClient();
  const environment = await client.environment.get(envId);
  ...
  const existing = environment.variables.find(variable => variable.name === varName);
  if (existing) {
    await environment.updateVariable(varName, { value });
  } else {
    await environment.addVariable({ name: varName, value, kind: "PLAIN" });
  }
