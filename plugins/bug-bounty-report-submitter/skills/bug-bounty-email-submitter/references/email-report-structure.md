...
Format that gist line as `PoC and logs: [url](url)`.
Put the `asciinema` URL on the next non-empty line below the gist URL as `PoC runtime: [url](url)`.
...
- the opening summary paragraph should already contain that gist URL as `PoC and logs: ...`, and the next non-empty line should contain the `asciinema` URL as `PoC runtime: ...`, so the recipient can open the canonical proof pack and recorded replay immediately
