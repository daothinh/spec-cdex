...
| `skills/bug-bounty-report-submitter/references/asciinema-proof.md` | Mandatory `asciinema` replay recording and link placement |
...
| `skills/bug-bounty-report-submitter/scripts/record_asciinema_replay.py` | Record the final clean replay with system `asciinema`, keep the local cast, upload it, and save metadata |
...
3. Let Codex inspect the live surface with Playwright MCP, build `bug-bounty-reports/<slug>/<finding-id>/`, carry the `reverify.md` verdict, `severity.md` triage, and the automatic `preverify.md` / `preverify-gate.json` gate into the bundle, then draft, submit, or send and capture the confirmation state.
4. Before drafting, automatically run the preverify gate. It must block AI slop, require a standalone PoC code file, require decisive output logs, and reject any replay path that still depends on source-tree `/test`.
5. Draft first. Then check `asciinema --version`. If native `asciinema` is missing, WSL may be used as fallback in this environment. If both checks fail, stop. Record the last clean standalone replay with `record_asciinema_replay.py`, then run `prepare_report_artifacts.py` so `artifacts.json`, `evidence/`, `evidence/poc/`, `evidence/logs/`, and `evidence/asciinema/` exist. Run `prepare_web3_report_bundle.py` when web3-specific report files still need to be materialized.
6. Treat `poc.md` as incomplete until it contains an exact run command or deterministic replay sequence plus a decisive success signal. Every report bundle must also carry a secret gist link containing the detailed PoC, helper files, the full report, and raw logs, plus an uploaded `asciinema` URL for the recorded replay. The two most important gist artifacts are mandatory: the standalone PoC code file and the decisive output-log file. Build `proof-pack/` with `prepare_external_proof_pack.py`, add an `external_proof` block to `submission.json` or `mail-envelope.json`, and include the gist URL in the report body with the `asciinema` URL immediately below it.
7. Run `validate_submission_bundle.py --bundle-dir <bundle-dir> --channel form|email` before final submit or send. If the payload or report body omits the required gist link, the `asciinema` link, the standalone PoC code file, the decisive output-log file, or the required link order, stop instead of sending an incomplete report.
8. Stop if the proof still depends on unresolved domain-logic or cryptographic assumptions or if the automatic preverify gate does not pass cleanly.
9. Draft the detail body from the shared evidence-first scaffold before adapting it to the live form or mailbox, then rewrite it into platform-native prose so the final report always shows the exact bug location, root cause, exploit path, runnable standalone replay, PoC output, and minimal exploit excerpt without reading like a reusable template. The finished report should read like a careful whitehat note written for one program, not an AI-generated bulk submission.
10. Keep the final report self-contained. Only upload attachments when the platform genuinely requires them or when a supplemental artifact adds decisive evidence. If a file is mandatory, prefer a markdown appendix such as `report-appendix.md` over raw code files.
