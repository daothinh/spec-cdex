...
| `skills/bug-bounty-report-submitter/references/external-proof-pack.md` | Workflow for secret-gist and field-limit proof-pack handling |
...
| `skills/bug-bounty-report-submitter/scripts/prepare_external_proof_pack.py` | Build a gist-ready runnable PoC proof pack and optionally publish a secret gist with `gh gist create` |
...
5. Treat `poc.md` as incomplete until it contains an exact run command or deterministic replay sequence plus a decisive success signal. If the platform is too small for the full runnable PoC, build `proof-pack/` with `prepare_external_proof_pack.py` and optionally publish a secret gist.
6. Stop if the proof still depends on unresolved domain-logic or cryptographic assumptions. The manual 20-minute review is a hard gate, not a nicety.
7. Draft the detail body from the shared evidence-first scaffold before adapting it to the live form or mailbox, then rewrite it into platform-native prose so the final report always shows the exact bug location, root cause, exploit path, runnable replay, PoC output, and minimal exploit excerpt without reading like a reusable template.
8. Keep the final report self-contained. Only upload attachments when the platform genuinely requires them or when a supplemental artifact adds decisive evidence. If a file is mandatory, prefer a markdown appendix such as `report-appendix.md` over raw code files.
