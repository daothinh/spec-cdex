...
4. Before drafting, check `asciinema --version`. If native `asciinema` is missing, WSL may be used as fallback in this environment. If both checks fail, stop. Record the last clean replay with `record_asciinema_replay.py`, then run `prepare_report_artifacts.py` so `artifacts.json`, `evidence/`, and `evidence/asciinema/` exist. Run `prepare_web3_report_bundle.py` when web3-specific report files still need to be materialized.
5. Treat `poc.md` as incomplete until it contains an exact run command or deterministic replay sequence plus a decisive success signal. Every report bundle must also carry a secret gist link containing the detailed PoC, helper files, the full report, and raw logs, plus an uploaded `asciinema` URL for the recorded replay. Build `proof-pack/` with `prepare_external_proof_pack.py`, add an `external_proof` block to `submission.json` or `mail-envelope.json`, and include the gist URL in the report body with the `asciinema` URL immediately below it.
6. Run `validate_submission_bundle.py --bundle-dir <bundle-dir> --channel form|email` before final submit or send. If the payload or report body omits the required gist link, the `asciinema` link, or the required link order, stop instead of sending an incomplete report.
