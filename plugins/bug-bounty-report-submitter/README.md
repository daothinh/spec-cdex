...
5. Treat `poc.md` as incomplete until it contains an exact run command or deterministic replay sequence plus a decisive success signal. Every report bundle must also carry a secret gist link containing the detailed PoC, helper files, and raw logs. Build `proof-pack/` with `prepare_external_proof_pack.py`, add an `external_proof` block to `submission.json` or `mail-envelope.json`, and include the gist URL in the report body itself.
