...
class BootstrapTargetTests(unittest.TestCase):
    def test_whitebox_bootstrap_clones_repo_and_downloads_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue((target_root / "prep" / "severity-conditions.md").exists())
            ...
            self.assertTrue((target_root / "prep" / "preverify-trigger.md").exists())
            ...
            self.assertTrue((target_root / "prep" / "context-pack" / "severity-conditions.md").exists())
            ...
            self.assertIn("medium", target_json["severity_conditions"])
            ...
            self.assertTrue(stdout["severity_conditions_file"].endswith("prep/severity-conditions.md"))
            ...
            self.assertIn("scope-check.md", (target_root / "findings" / "README.md").read_text(encoding="utf-8"))
            ...
            self.assertIn("Severity Conditions", (target_root / "prep" / "bootstrap-summary.md").read_text(encoding="utf-8"))
            self.assertNotIn("Next Best Attack Path", (target_root / "prep" / "bootstrap-summary.md").read_text(encoding="utf-8"))
            ...
            self.assertIn("Typical medium+ web outcomes", (target_root / "prep" / "severity-conditions.md").read_text(encoding="utf-8"))
            ...
            self.assertIn(
                "automatic submit gate",
                (target_root / "prep" / "preverify-trigger.md").read_text(encoding="utf-8"),
            )
    ...
    def test_smart_contract_bootstrap_collects_contract_inventory(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("smart-contract", target_json["environment_readiness"]["profiles"])
            self.assertIn("critical", target_json["severity_conditions"])
            ...
            self.assertIn("Typical medium+ contract outcomes", (target_root / "prep" / "severity-conditions.md").read_text(encoding="utf-8"))
            self.assertIn("signature", (target_root / "prep" / "domain-logic.md").read_text(encoding="utf-8"))
            self.assertIn("preverify.md", (target_root / "findings" / "README.md").read_text(encoding="utf-8"))
            self.assertIn("preverify-gate.json", (target_root / "findings" / "README.md").read_text(encoding="utf-8"))
